...
It tells me that for this fixed probability coin theta, it shows a valid probability distribution that the sum of them would be 1

```{r}
row_sum = prob_table(heads)

# Your code here: using a similar structure as above, compute row sums, mutate it (and excluding the theta column)

row_sums <- prob_table %>%
  mutate(row_sum = rowSums(across(starts_with("heads_"))))
row_sums %>% select(theta, row_sum)
```
...
No the likelihood does not sum to 1, this sum is 1.1445, and this tells me that a likelihood function is not a probability distribution over the theta, so it does not need to sum up to one.
...
Yes, summing the posterior column is 1, and this tells me that it is the probability distribution, so it must sum up to one as the total probability.
...
posterior_table %>%
  ...
  summarise(prob = sum(posterior), percent = 100 * prob)
```

There’s a 57.5% chance that the theta is 0.5 or 0.6.
...
comparing this posterior to flat-prior posterior,the theta 0.5 is the most probable now. The prior did pull the estimation.
...
The posterior shift is because the prior changed to close to 0.5, so the posterior would shift toward it as well. Priors matter most when data are small and the posterior would matter more when there is 500 flips.
...
The peak of the posterior is at around 0.6, and this is not matching the MLE from section 2.
...
The posterior sits between the prior and the likelihood is because it is the produced of the prior and likelihood, so it compromise between these two, thus sits between them. If I had Y = 300 heads out of N = 500 instead of Y = 3 of N = 5, the posterior would move closer to the likelihood peak.
...
As the n increases, the posterior would has a greater peak and become narrower in its peak range. The pior would matter very little at n = 200 since the likelihood would be more informative compared to the prior when the n = 200.
...
The posterior mean of theta is 0.598. It is closer to 0.6, and this is because the posterior driven by likelihood, so closer to 0.6.

### Question 9.2

How does this compare to your grid approximation result from Section 5?

This is consistent with the grid approximation result from previous section since it is closer to 0.6.
...
The MCMC posterior histogram has essentially the same shape as the grid approximation posterior. The main difference the MCMC histogram is noisißer.
...
The posterir mean shifted in the firection toward 0.5, and this is because threr is only several flips, so the prior has a stronger power with limited data. This matches what happened in previous section.
...
If I have 500 flips, I would expect the likelihood get very sharp, and the posterior become very similar to each other, both of them would center around 0.6, and narrower.
...
This is not directly comparable to the grid approximation posterior at theta = 0.6, because previous is an interval probability while the latter is a point probability for a discrete set of values.
...
The theta = 0.5 is inside of the 95% credible interval, and the thea of 0.2 is also within the interval, and that means they are plausible values under the posterior given the actual data and prior information.
