...
ggplot(d, aes(x = reorder(tank, propsurv), y = propsurv, fill = factor(density))) +
  ...
  scale_fill_manual(
    values = c("10" = "steelblue", "25" = "coral", "35" = "seagreen"),
    name = "Initial density"
  ) +
  labs(
    x = "Tank",
...
#So my observations is flliped here. But if smaller tanks do produce extreme estimates, that would because of the smaller sample size, and each of the tappoles would have larger impact on the variability. 
```
...
```{r}
# Enter your answer here
# the glm() coefficient and the brm() posterior mean are no that similar to each other? I would expect them to be similar to each other. 
```
...
fit_prior_wide <- brm(
  ...
  data = d,
  sample_prior = "only",
  file = "fits/lab4_fit_prior_wide"
)
pp_check(fit_prior_wide, ndraws = 100) +
  theme_bw(base_size = 14) +
  labs(title = "Wider SD Prior Prior predictive check")
```
...
```{r}
# Enter your answer here
# Complete pooling assumes that all 48 tanks are having the  exactly the same underlying survival probability. 
```
...
no_pool_estimates %>%
  mutate(tank_num = as.integer(tank)) %>%
  left_join(d %>% mutate(tank_num = as.integer(tank)) %>% select(tank_num, density),
            by = "tank_num") %>%
  filter(tank_num %in% c(1, 37)) %>%     # tank 1 (density 10), tank 37 (density 35)
  select(tank, density, prob, prob_lower, prob_upper) %>%
  mutate(interval_width = prob_upper - prob_lower)
```
...
```{r}
# Enter your answer here
#It is the standard deviation of the normal population distribution from which the tank-level intercepts are drawn. If it were very close to 0, the tank-level estimates would have no variations between tanks, and if it were very large, the tanks should independent. 
```
...
```{r}
# Enter your answer here
#Running the summary(fit_complete) ket me see the raht values are bvelow 1.01. Bulk_ESS and Tail_ESS are above 400, and i think the trace plots shwo good mixing. 
```
...
```{r}
# Enter your answer here
#The shrinkage plot shows that many points fall between the
# diagonal and the red dotted grand-mean line. The green tanks show the largest movement, and i think shrinkage is strongers to large-sample tanks.
...
```{r}
# Enter your answer here
# The partial pooling model pulls that tank's estimate down from 1.0 toward the grand mean  because the model recognizes that extreme proportions from small(or larger?) samples sampling noise.
```
...
```{r}
# Enter your answer here
# glmer() estimates the group-level standard deviation as a single point. The intervals are depdndent on a fixed variance. This fundamental different is that Bayesian credible intervals are wider bout total uncertainty. 
```
...
```{r}
# Enter your answer here
# The Bayesian (blue) credible intervals are generally slightly wider than the frequentist (red) intervals. 
```
...
```{r}
# Enter your answer here
# The posterior predictive distribution captures the general shape of the observed data (dark line) reasonably well (I am actually very surprised!). Theew ia no systematic discrepencies? 
```
...
```{r}
# Enter your answer here
#ELPD has the larger difference comparedto is standrard error. The partial pooling wins on LOO-CV.
```
...
plot(loo_partial, label_points = TRUE)
```
