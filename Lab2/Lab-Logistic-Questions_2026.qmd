...
ggplot(polviews_summary, aes(x = polviews, y = proportion)) +
  ...
  theme_minimal() +
  labs(title = "DistributionPlots", x = "Political Views", y = "Proportion")
  
```

> Which political view occurs most frequently in this data set?
>
> Moderate occurs most frequently i this dataset.
...
```{r}
satisfaction_by_view <- data %>%
  group_by(polviews) %>%
  summarize(prop_satisfied = mean(mass_trans_spend_right)) 

ggplot(satisfaction_by_view, aes(x = polviews, y = prop_satisfied)) +
  geom_col(fill = "steelblue") +
  theme_minimal() +
  labs(title = "Satisfaction with Mass Transit Spending and Political View",
       x = "Political Views",
...
ggplot(data, aes(x = age)) +
  ...
  theme_minimal() +
  labs(title = "Frequency Distribution of Age",x = "Age",y = "Count")
```
...
data.frame(
  Probability = b0_transformed,
  CI_Lower = p_lower,
  CI_Upper = p_upper
) %>% kable()
```

Interpretation: The intercept's corredposnding probability is 52.9, with a confidence interval from around 51 to 55.
...
data <- data %>%
  mutate(
    ...
    sei10_centered = sei10 -mean(sei10, na.rm = TRUE)
  )


#fit with glm()
demog_model <- glm(
  mass_trans_spend_right ~ age_centered + sex + sei10_centered,data = data,family = binomial
)
...
bage <- coef(m1)["age_centered"]
se_age <- summary(m1)$coefficients["age_centered", "Std. Error"]
...
```{r}
bses <- coef(m1)["sei10_centered"]
se_ses <- summary(m1)$coefficients["sei10_centered", "Std. Error"]
...
A one unit increase in SES index is associated with a decrease in the log-odds of being satisfied with spending on mass transportation by 0.0062 units (95% CI \[-0.0095, -0.003\]), holding all other variables constant. The odds ratio is 0.9938 (95% CI \[0.9906, 0.997\], which confirms the negative relationship implied by the log-odds coefficient. Specifically, for each additional unit of SES index, the odds of being satisfied with mass transportation spending change by a factor of about 0.994, or approximately a 0.62% per unit increase in SES index, holding other factors constant.
...
```{r}
ame_results <- avg_comparisons(m1, comparison = "difference") 
...
-   The marginal effect for being female compared to male is 0.0631 (95% CI \[0.0246, 0.1016\]). This indicates that females are, on average, about 6.31 percentage points more likely than males to be satisfied with mass transportation spending, holding other factors constant.
...
fit2 <- glm(mass_trans_spend_right ~ age_centered + sex + sei10_centered, 
            data = data, 
            family = binomial)

fit3 <- glm(mass_trans_spend_right ~ age_centered + sex + sei10_centered + polviews, 
            data = data, 
            family = binomial)
...
-   We next use the model to produce visualizations:
    ...
    -   Given the code below, interpret what is being plotted:
        ...
        -   pol_plot : this plot plots thee mass transportation spending is "about right" across the different levels of political view

        -   sex_plot : this plot plots being satisfied with mass transportation spending for females compared to males

        -   ses_plot: this plot plots the relationship between Socioeconomic Status (SES) and the satisfaction
...
# 1. Define Colors
# Note: Spellings must match your data EXACTLY (including the typos in the CSV)
colors <- c("Extremely liberal" = "black",
            "Liberal" = "#0e2f44",
            "Slightly liberal" = "#1d5a6c",
            "Moderate" = "#358ca3",
            "Slghtly conservative" = "#71b9d1", # Matches data typo
            "Conservative" = "#a6dcef",
            "Extrmly conservative" = "#d0f0fd") # Matches data typo

# 2. Political Views Plot
...
pol_plot <- ggplot(pp_pol, aes(x = x, y = predicted, color = x)) +
  ...
  labs(title = "Satisfaction with Mass Trans. Spending by Political Views",
       subtitle = "Predicted probabilities holding Age, Sex, and SES constant",
       x = "Political Views", 
       y = "Predicted Probability",
       color = "Political Views") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) # Angled text for readability
...
sex_plot <- ggplot(pp_sex, aes(x = x, y = predicted, color = x)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2, linewidth = 0.8) +
  labs(title = "Satisfaction with Mass Trans. Spending by Sex",
       subtitle = "Predicted probabilities holding Age, SES, and PolViews constant",
       x = "Sex", 
       y = "Predicted Probability",
       color = "Sex") +
  theme_minimal() +
...
# 4. SES Plot
# CRITICAL FIX: Use "sei10_centered" because that is what is in fit3
pp_ses <- ggemmeans(fit3, terms = "sei10_centered [all]")

ses_plot <- ggplot(pp_ses, aes(x = x, y = predicted)) +
  geom_line(color = "#2c7fb8", linewidth = 1) + 
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "#2c7fb8", alpha = 0.2) +
  labs(title = "Satisfaction with Mass Trans. Spending by SES",
       subtitle = "Predicted probabilities holding Age, Sex, and PolViews constant",
       x = "Socioeconomic Status (Centered)", 
       y = "Predicted Probability") +
  theme_minimal() +
...
::: callout-note
Answer: The logistic model might not be a good choice here because ideally about 95% of the binned residuals should fall within the error bounds, but the reality is only about 86% falls in the error bounrds.
:::
...
> R2 interpretation: This shows that the model explains only about 1.0% of the variation in being satisfied with mass transportation spending.
>
...
binned_residuals(fit2, term="sei10_centered") %>% plot(show_dots=TRUE)
binned_residuals(fit2, term="age_centered") %>% plot(show_dots=TRUE)
...
fit2_improved <- glm(
  mass_trans_spend_right ~ age_centered + sex + sei10_centered + I(sei10_centered^2),
  data = data,
  family = binomial
)
...
binned_residuals(fit2_improved, term = "sei10_centered") %>% plot(show_dots = TRUE)
```

::: callout-note
Did adding the squared term improve the model? No.
:::
...
-   Conservatives have 2.17times the odds of supporting mass transit spending compared to extremely liberal and liberal respondents

-   Extreme liberals have 0.37 times the odds of supporting spending compared to conservatives, moderates, and slightly conservative respondents

-   Extremely conservative respondents have 2.08 times the odds of supporting mass spending compared to liberals and slightly liberal respondents

-   Liberals have 0.49 times the odds of supporting spending compared to moderates and slightly conservative respondents

## Conclusion

The conclusions is that SES, political views, age and sex, are all significant predictors of satisfaction with mass transportation spending based on the model summary below, and the three figures.
