...
#The marginal histograms added by ggside shows me that the body_mass_g follows a bimodal distribution, and the bill_length_mm follows also a multimodal and a little skewed distribution.
...
```{r}
# Enter your code here (mean of the residuals)
...
```{r}
# Enter your answer here
# the lm() output is about sampling distribution and hypothesis testing, and the brm()output shows the posterior estimates.  
```
...
predict(fit_gentoo_ols,
  ...
  geom_point(data = gentoo,
             aes(y = bill_length_mm)) +
  labs(title = "Gentoo: OLS regression",
       x = "Body mass (g)",
       y = "Bill length (mm)")
...
conditional_effects(fit_gentoo_b)
...
bind_rows(
  ...
  scale_y_discrete(expand = expansion(mult = 5)) +
  facet_wrap(~ term, scales = "free_x") +
  labs(title = "Gentoo coefficient comparison")
...
```{r}
# Enter your answer here
#the diagonal distribution of the above plot is the marginal posterior distribution. The dsitribution in the off-diagonal is the joint posterior distributions.
```
...
```{r}
# Enter your answer here
#This relates to representing uncertainty because they have 4000 plausibe values sampled from the posterior distribution of the parameters. Predictions are like y_hat_i = beta_0 + beta_1 * body_mass_g_i
```
...
```{r}
# Enter your answer here
#The posterior for sigma is right-skewed because the sigma cannot go below 0, but it can take a range of larger positive values. So very small values are constrained, while somewhat larger
# values are still plausible, which stretches the posterior to the right.
...
```{r}
# Enter your answer here
#The implication of using just the mean is that the mean may be larger than the normal value when the posterior is right-skewed. The median might be more robust to make s prediction here. And for the mode of posteriors, it may ignore broader shape of the uncertainty.
```
...
```{r}
# Enter your answer here
# I think the brms is still using priors even we did not specify  a prior. So we are doing the beyasian with the defualts here. 
```
...
gentoo_draws <- as_draws_df(fit_gentoo_b)
...
gentoo_draws %>%
  ...
  geom_histogram(bins = 40) +
  facet_wrap(~ name, scales = "free") +
  labs(title = "Gentoo posterior draws")

# Conditional effects plot
conditional_effects(fit_gentoo_b)

# A Gentoo spaghetti plot
gentoo_draws %>%
  mutate(beta0 = b_Intercept,
         beta1 = b_body_mass_g) %>%
  slice_sample(n = 20) %>%
