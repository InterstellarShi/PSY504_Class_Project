...
ggplot(ratings, aes(x = latent)) +
  geom_density(fill = "skyblue", color = "steelblue", linewidth = 1) +
  geom_vline(xintercept = tau_psych, linetype = "dashed", color = "orange", linewidth = 0.9) +
  annotate("text", x = -2.0, y = 0.10, label = "Poor") +
  annotate("text", x = -0.5, y = 0.18, label = "Fair") +
  annotate("text", x =  0.5, y = 0.18, label = "Good") +
  annotate("text", x =  1.8, y = 0.10, label = "Excellent") +
  labs(
    title = "Latent scores for Psychology thresholds",
    x = "Latent scores",
    y = "Density"
  )
...
rating_dist <- bind_rows(psych_counts, cs_counts) %>%
  group_by(dept) %>%
  mutate(prop = n / sum(n)) %>%
  ungroup()

ggplot(rating_dist, aes(x = dept, y = prop, fill = category)) +
  geom_col(position = "stack", width = 0.7) +
  scale_y_continuous(labels = percent_format()) +
  labs(
    title = "Observed rating distributions for Psychology and CS",
    x = "Department",
    y = "Proportions",
    fill = "Rating"
  )+
...
tibble(
  threshold = c("tau1", "tau2", "tau3"),
  value = thresholds,
  cumulative_probability = cumul_probs)%>%
  kable(caption = "Thresholds converted to cumulative probs")
)
  
tibble(
  category = c("Poor", "Fair", "Good", "Excellent"),
  probability = cat_probs) %>%
  kable(caption = "Category-specific probabilities")
)
# (b) Desired split to thresholds
desired_split <- c(0.10, 0.30, 0.40, 0.20)
cumul_desired <- cumsum(desired_split[1:3])  
needed_thresholds <- qlogis(cumul_desired)

tibble(
  cumulative_cut = cumul_desired,
  required_threshold = needed_thresholds
) %>% kable(digits = 3, caption = "Thresholds needed for desired category split")
```
...
Each of the model model threshold is the same as the hand-calculated log-cumulative-odds.

```{r}
m0 <- clm(eval_psych ~ 1, data = ratings)
summary(m0)
thr_model <- coef(m0)[grep("\\|", names(coef(m0)))]
thr_model
hand <- ratings %>%
  count(eval_psych) %>%
  rename(category = eval_psych) %>%
  mutate(
    p = n / sum(n),
    cum_p = cumsum(p),
    log_cum_odds = log(cum_p / (1 - cum_p))
  )

hand
compare <- tibble(
  threshold = names(thr_model),
  model = as.numeric(thr_model),
  hand = hand$log_cum_odds[1:3]
)
...
The gray bar is P(Y smaller or euqla to Fair), and the blue bar is the P(Y = Fair).
...
In binary logistic regression, the logit models one probability, in the ordinal logit, it models cumulative probabilities. And there are K-1 equations to be produced.
...
```{r}
df_1234 <- ratings %>%
  mutate(rating_num = as.numeric(eval_psych)) %>%   
  group_by(id) %>%
  summarise(
    effectiveness = first(effectiveness),
    rating_mean = mean(rating_num),
    .groups = "drop"
  )

lm_1234 <- lm(rating_mean ~ effectiveness, data = df_1234)
slope_1234 <- coef(lm_1234)["effectiveness"]
...
lm_12310 <- lm(rating_mean ~ effectiveness, data = df_12310)
slope_12310 <- coef(lm_12310)["effectiveness"]
...
The slopes for Coding **Poor=1, Fair=2, Good=3, Excellent=4** is 0.4518, and the slopes for **Poor=1, Fair=2, Good=3, Excellent=10** is 1.7059. They are different because lnear regression treats the coding categories as metric scale with equal spacing, but this is not the same way as the ordinal categories do.
...
#| code-fold: false
#a
p_gender <- ggplot(gb, aes(x = Gender, fill = Rank)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
    title = "Proportion of technical ranks by gender",
    x = "Gender",
    y = "Proportion",
    fill = "Rank"
  ) +
...
gb_age <- gb %>%
  mutate(
    AgeGroup = cut(Age, breaks = c(15, 25, 35, 45, 55, 75), include.lowest = TRUE, right = FALSE)
  )

p_age <- ggplot(gb_age, aes(x = AgeGroup, fill = Rank)) +
  geom_bar(position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
    title = "Proportion of technical ranks by age group",
    x = "Age group",
    y = "Proportion",
    fill = "Rank"
  ) +
...
m2_thresholds <- tibble(
  term = names(coef_m2),
  estimate = as.numeric(coef_m2)
) %>%
  filter(str_detect(term, "\\|"))

m2_thresholds %>% kable(digits = 3, caption = "Model 2 threshold estimates")
...
m2_or %>% kable(digits = 3, caption = "Model 2 coefficients as odds ratios")
```

```{r}
#| code-fold: false
#| c
pred_30F <- predictions(
  m2, 
  newdata = datagrid(Gender = "F", Age = 30),
  type = "prob"
)

pred_30F %>% kable(digits = 3, caption = "Predicted probabilities for a 30-year-old female (Model 2)")
```
...
Yes, the likelihood ratio test is significant.
...
```{r}
pred_25_compare <- predictions(
  m2,
  newdata = datagrid(Gender = c("F", "M"), Age = 25),
  type = "prob"
)

pred_25_compare %>% 
  kable(digits = 3, caption = "Predicted probabilities")
```
...
coefs <- coef(m2)
coefs
alpha_3 <- coefs[grep("^3\\|2$", names(coefs))]

beta_age <- coefs["Age"]

gender_term_name <- names(coefs)[str_detect(names(coefs), "^Gender")]
beta_gender <- if (length(gender_term_name) == 0) 0 else coefs[gender_term_name]

x_gender_female <- 0
x_age <- 25
p_y_leq_3_manual <- plogis(alpha_3 - (beta_gender * x_gender_female + beta_age * x_age))

manual_check <- tibble(
  quantity = "P(Y<= 3) for female age 25",
  manual_value = as.numeric(p_y_leq_3_manual)
)

manual_check %>% kable(digits = 6, caption = "Manual cumulative probability")

# Compare to model output: because 3 is the lowest category, P(Y <= 3) = P(Y = 3)
pred_25F_model <- predictions(
  m2,
  newdata = datagrid(Gender = "F", Age = 25),
  type = "prob"
)

pred_25F_model %>% kable(digits = 6, caption = "Model predicted probabilities")
```
...
```{r}
ame_age <- avg_comparisons(m2, variables = "Age", type = "prob")

ame_age %>% kable(digits = 4, caption = "Average marginal effect of Age")
```

------------------------------------------------------------------------

Growingone year older is associated with about a 0.12 increase in the probability of finishing rank 1 .
...
```{r}
ggem_df <- ggemmeans(m2, terms = c("Age", "Gender"))
...
brant_results <- brant.test(m2_polr)
brant_results
...
The latent normality assumption looks largely good since the surrogate residual QQ plot is approximately linear through the center.

```{r}
#| code-fold: false
sur_res <- sure::resids(m2)
qqnorm(sur_res, main = "QQ plot of surrogate residuals")
qqline(sur_res)
```
...
We used a cumulative logit ordinal regression model (proportional odds to predict technical rank (1st, 2nd, 3rd) from gender and age (N = 309). A Brant test supported the proportional odds assumption (χ² = 0.367, p = 0.83). Gender was not a significant predictor of rank (OR = 0.826, 95% CI \[0.546, 1.248\], p = 0.365). Age was also not a significant predictor (OR = 1.006, 95% CI \[0.988, 1.024\], p = 0.537). In probability terms, the average marginal effect of switching from female to male decreased the probability of Rank 1 by about 4.2 percentage points.
