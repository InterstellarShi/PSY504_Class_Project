...
#The model gets correct at the part that close to the peak value and the rough estimate of the peak; however, the normal curve just have a assumption of the guassian distribution while the actual distribution is unimodal but it is unfortunately not a gaussian distribution.
```
...
```{r}
# Enter your answer here
#Compared to the fitted, they they conditional posterior mean, the predcit() add posteror prediction so that the predict() band is always wider. For fitted(), I would report it when I want to know the current mean for the body mass data I have, and for the predict(), I would report it when I need some posterior prediction such as I want to know what would be the potential future mean of the data, so basically to simulate the data. 
```
...
```{r}
# Enter your answer here
#The pp_check() is trying to simulatenew datasets from the posterior predictive distribution and then comparing those simulated datasets to the observed data, it is doing the same thing as the density overlay under the hood. 
```
...
```{r}
# Enter your answer here
#The prediction looks reasonable. If the model predicted bill lengths of 500 mm, so the priors are pretty unrealisic, so I would make the prior to change so it would make sense biologically. 
```
...
```{r}
# Enter your answer here
#The priors in fit1.b and fit2.b is pretty similar. If the results are nearly identical, that means that priors being set explicity is eproducing the defaults, so the model fit does not change much. 
```
...
```{r}
# Enter your answer here
#In practical terms, this tells me that body_mass_g is a useful predictor of bill_length_mm.In practical terms, knowing a penguin’s body mass helps improve our predictions of its bill length.
```
...
summary(fit_gentoo.b)

conditional_effects(fit_gentoo.b) %>% 
  plot(points = TRUE)
...
fitted(fit_gentoo.b, newdata = nd_gentoo) %>% 
  data.frame() %>% 
  bind_cols(nd_gentoo) %>% 
  ggplot(aes(x = body_mass_g)) +
...
pp_check(fit_gentoo.b, ndraws = 100)

# Figure 4: parameter distributions
as_draws_df(fit_gentoo.b) %>% 
  rename(`beta[0]` = b_Intercept,
         `beta[1]` = b_body_mass_g) %>% 
  pivot_longer(cols = c(`beta[0]`, `beta[1]`, sigma),
               names_to = "parameter") %>% 
